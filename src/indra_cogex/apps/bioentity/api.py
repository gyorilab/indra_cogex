...
uniprot_mnemonic_ids_model = bioentity_ns.model('get_uniprot_ids_from_uniprot_mnemonic_ids_model', {
    'uniprot_mnemonic_ids': fields.List(fields.String, required=True, description='List of UniProt mnemonic IDs', example=['CLH1_HUMAN'])
})
uniprot_ids_model = bioentity_ns.model('get_hgnc_ids_from_uniprot_ids_model', {
    'uniprot_ids': fields.List(fields.String, required=True, description='List of UniProt IDs', example=['Q00610'])
})
...
class UniprotIdsFromUniprotMnemonicIds(Resource):
    ...
    -------
    post():
        Receives a JSON payload with a list of UniProt mnemonic IDs and returns a mapping of these IDs to their corresponding UniProt IDs.
    """
    ...
    def post(self):
        ...
        This method expects a JSON payload with a key "uniprot_mnemonic_ids" containing a list of UniProt mnemonic IDs.
        It uses the UniProt client to convert each mnemonic ID to a UniProt ID.
        Then, it adds the mapping from the mnemonic ID to the UniProt ID to the result.

        Returns:
            dict: A dictionary mapping UniProt mnemonic IDs to UniProt IDs.
        """
...
class HgncIdsFromUniprotIds(Resource):
    ...
    -------
    post():
        Receives a JSON payload containing a list of UniProt IDs and returns a mapping of UniProt IDs to HGNC IDs.
    """
    ...
    def post(self):
        ...
        This method expects a JSON payload with a list of UniProt IDs under the key "uniprot_ids".
        It returns a dictionary mapping each UniProt ID to its corresponding HGNC ID, if available.

        Returns:
            dict: A dictionary where keys are UniProt IDs and values are HGNC IDs.
        """
...
class HgncNamesFromHgncIds(Resource):
    ...
    def post(self):
        ...
        This method expects a JSON payload with a list of HGNC IDs under the key "hgnc_ids".
        It retrieves the corresponding HGNC names using the `hgnc_client` and returns a 
        dictionary mapping each HGNC ID to its name.

        Returns:
            dict: A dictionary where keys are HGNC IDs and values are HGNC names.
        """
...
class CheckIfKinase(Resource):
    ...
    -------
    post():
        Receives a JSON payload with a list of genes and returns a mapping of each gene to a boolean indicating if it is a kinase.
    """
    ...
    def post(self):
        ...
        It checks each gene to determine if it is a kinase using the `hgnc_client.is_kinase` method
        and returns a mapping of each gene to a boolean indicating whether it is a kinase.

        Returns:
            dict: A dictionary where the keys are gene names and the values are booleans indicating
                  whether each gene is a kinase.
        """
...
class CheckIfPhosphatase(Resource):
    ...
    -------
    post():
        Receives a JSON payload with a list of genes and returns a mapping of each gene to a boolean indicating if it is a phosphatase.
    """
    ...
    def post(self):
        ...
        This method expects a JSON payload with a list of gene identifiers under the key "genes".
        It checks each gene to determine if it is a phosphatase using the `hgnc_client.is_phosphatase` method.
        The result is a dictionary mapping each gene to a boolean indicating whether it is a phosphatase.

        Returns:
            dict: A dictionary where keys are gene identifiers and values are booleans indicating if the gene is a phosphatase.
        """
        genes = request.json["genes"]
        mapping = dict()
        for gene in genes:
...
class CheckIfTranscriptionFactor(Resource):
    ...
    Methods:
        post():
            Receives a JSON payload with a list of genes and returns a mapping of each gene to a boolean indicating
            whether it is a transcription factor.
    """
    ...
    def post(self):
        ...
        Returns:
            dict: A dictionary where keys are gene names and values are booleans
                  indicating whether the gene is a transcription factor.
        """
        genes = request.json["genes"]
        mapping = dict()
        for gene in genes:
