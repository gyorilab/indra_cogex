...
def do_cypher_tx_with_keys(
    ...
    """Execute Cypher query and return both column names and values.
    ...
    :
        Tuple of (column_names, rows) where:
        - column_names: List of column names from RETURN clause
        - rows: List of value lists (same as do_cypher_tx)
    """
    result = tx.run(query, parameters=query_params)
    # Consume first record to get keys, then build full result
    records = list(result)
    if not records:
        return [], []
