...
def df_binomtest(
    df: pd.DataFrame, c1: str, c2: str, sort: bool = True, sign: bool = True,
    method: str = "fdr_bh",
...
def pipeline1(client: Neo4jClient):
    ...
    for target_curie, target_name in [
        ...
        for _, _, stmt, annotations in _get_res(
            client=client, source_curie=source_curie, target_curie=target_curie,
        ):
            for annotation in annotations:
                mesh_id = annotation["identifier"].removeprefix("mesh:")
                if not is_disease(mesh_id):
                    continue
                annotations_dd[target_curie, target_name][mesh_id, annotation["name"]].add(annotation["publication"])
