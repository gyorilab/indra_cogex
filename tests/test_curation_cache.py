...
class TestCurationCache(unittest.TestCase):
    def test_get_incorrect_evidence_hashes(self):
        curations = [
            dict(source_hash=1, tag="correct", date=TEST_DATE),
            dict(source_hash=1, tag="incorrect", date=TEST_DATE),
            dict(source_hash=2, tag="correct", date=TEST_DATE),
            dict(source_hash=3, tag="incorrect", date=TEST_DATE),
        ]
    ...
    def test_get_correct_statement_hashes(self):
        curations = [
            dict(pa_hash=1, tag="correct", date=TEST_DATE),
            dict(pa_hash=1, tag="incorrect", date=TEST_DATE),
            dict(pa_hash=2, tag="correct", date=TEST_DATE),
            dict(pa_hash=3, tag="incorrect", date=TEST_DATE),
        ]
        curation_cache = MockCurationCache(curations)
