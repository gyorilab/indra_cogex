...
class TestSQLiteCache(unittest.TestCase):
    def setUp(self):
        ...
        # Build the cache
        self.queried_cache = build_sqlite_cache(
    ...
    @pytest.mark.nogha
    def test_0_cache_built(self):
        self.assertTrue(TEST_SQLITE_CACHE_PATH.exists())

    @pytest.mark.nogha
    def test_1_cache_contents(self):
        import sqlite3
    ...
    def _check_gene_set_cache_equality(self, dataset_name: str, sqlite_data: dict):
        data = self.queried_cache[SQLITE_GENE_SET_TABLE][dataset_name]
        self.assertIsInstance(data, dict)
        # Check that the keys are the same
        self.assertEqual(set(data.keys()), set(sqlite_data.keys()))
        # Check that the values are the same (values are sets)
        for key, val in data.items():
            self.assertEqual(val, sqlite_data[key])

    def _check_genes_with_confidence_cache(
        self, dataset_name: str, sqlite_data: dict
    ):
        data = self.queried_cache[SQLITE_GENES_WITH_CONFIDENCE_TABLE][dataset_name]
        self.assertIsInstance(data, dict)
        # Check that the keys are the same
        self.assertEqual(set(data.keys()), set(sqlite_data.keys()))
        ...
        for key, val in data.items():
            ...
            self.assertEqual(set(val.keys()), set(sqlite_data[key].keys()))
            for inner_key, inner_val in val.items():
                self.assertEqual(inner_val, sqlite_data[key][inner_key])

    @pytest.mark.nogha
    def test_go_cache(self):
        sqlite_go_data = gene_set_table_datasets["go"](
            use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH
        )
        self._check_gene_set_cache_equality("go", sqlite_go_data)

    @pytest.mark.nogha
    def test_reactome_cache(self):
        sqlite_reactome_data = gene_set_table_datasets["reactome"](
            use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH
        )
        self._check_gene_set_cache_equality("reactome", sqlite_reactome_data)

    @pytest.mark.nogha
    def test_wikipathways_cache(self):
        sqlite_wikipathways_data = gene_set_table_datasets["wikipathways"](
            use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH
        )
        self._check_gene_set_cache_equality(
            "wikipathways", sqlite_wikipathways_data
        )

    @pytest.mark.nogha
    def test_phenotype_gene_cache(self):
        sqlite_phenotype_gene_data = gene_set_table_datasets["phenotypes"](
            use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH
        )
        self._check_gene_set_cache_equality(
            "phenotypes", sqlite_phenotype_gene_data
        )

    @pytest.mark.nogha
    def test_entity_to_targets_cache(self):
        sqlite_entity_to_targets_data = genes_with_confidence_datasets[
            "entity_to_targets",
        ](use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH)
        self._check_genes_with_confidence_cache(
            "entity_to_targets", sqlite_entity_to_targets_data
        )

    @pytest.mark.nogha
    def test_entity_to_regulators_cache(self):
        sqlite_entity_to_regulators_data = genes_with_confidence_datasets[
            "entity_to_regulators"
        ](use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH)
        self._check_genes_with_confidence_cache(
            "entity_to_regulators", sqlite_entity_to_regulators_data
        )

    @pytest.mark.nogha
    def test_positive_stmts_cache(self):
        sqlite_positive_stmts_data = genes_with_confidence_datasets[
            "positive_statements"
        ](use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH)
        self._check_genes_with_confidence_cache(
            "positive_statements", sqlite_positive_stmts_data
        )

    @pytest.mark.nogha
    def test_negative_stmts_cache(self):
        sqlite_negative_stmts_data = genes_with_confidence_datasets[
            "negative_statements"
        ](use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH)
        self._check_genes_with_confidence_cache(
            "negative_statements", sqlite_negative_stmts_data
        )

    @pytest.mark.nogha
    def test_kinase_phosphosites_cache(self):
        sqlite_kinase_phosphosites_data = genes_with_confidence_datasets[
            "kinase_phosphosites"
        ](use_sqlite_cache=True, sqlite_db_path=TEST_SQLITE_CACHE_PATH)
        self._check_genes_with_confidence_cache(
            "kinase_phosphosites", sqlite_kinase_phosphosites_data
        )
